...
public class Analyze {
   ...
   // LangIdent lgid = new LangIdent(DATA + "/common/lang_ident/ident-few.dat");
    ...
    try{
        ...
        for (String texto1 : texto){
            archivo = new File("/home/gorro/Escritorio/Textos/" + texto1 + "/" + texto1 + "_cn.txt");
  ...
  private static void printSenses( Word w, int i, String texto1) {
...
//    try
//        {
//            fichero = new FileWriter("/home/gorro/Escritorio/Textos/" + texto1 + "/" + texto1 +  "_cn" + i + ".txt",true);
...
//        }
  ...
  private static void printResults( ListSentence ls, String format, int i, String texto1 ) {
  ...
  /*private static void printParseTree( int depth, ParseTree tr ) {
    ...
    else
    {
      ...
      System.out.println( tr.begin().getLabel() + "_[" );
  ...
  private static void printDepTree( int depth, DepTree tr, int i, String texto1 ) {
    DepTree child = null;
    DepTree fchild = null;
    long nch;
    int last, min;
...
//            fichero = new FileWriter("/home/gorro/Escritorio/Textos/" + texto1 + "/" + texto1 +  "_cn" + i + ".txt",true);
//            pw = new PrintWriter(fichero);
//            pw.print(tr.begin().getLink().getLabel() + "/" +
//      tr.begin().getLabel() + "/" );
//            fichero.close();
// 
//        } catch (Exception e) {
...
//        }
    ...
    try
        {
            fichero = new FileWriter("/home/gorro/Escritorio/Textos/" + texto1 + "/" + texto1 +  "_cn" + i + ".txt",true);
            pw = new PrintWriter(fichero);
            pw.print( "(" + w.getForm() /*+ " " + w.getLemma() + " " + w.getTag()*/);
            fichero.close();
    ...
    if( nch > 0 ) {
      ...
      for( int j = 0; j < nch; j++ ) {
        child = tr.nthChildRef( j );

        if( child != null ) {
          if( !child.begin().isChunk() ) {
            printDepTree( depth + 1, child,i,texto1 );
          }
        }
        else {
          System.err.println( "ERROR: Unexpected NULL child." );
      ...
      while( trob ) {
        ...
        if( trob && (child != null) ) {
          printDepTree( depth + 1, fchild ,i,texto1);
        }
